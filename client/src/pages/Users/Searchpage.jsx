...
import { SlidersHorizontal, RotateCcw, Image as ImageIcon, Search } from 'lucide-react';
...
const Searchpage = () => {
  const { searchQuery, setSearchQuery, getCars, isLoading } = useCarStore();
  ...
  return (
    <div className="space-y-6 animate-in fade-in duration-700 pb-10 relative" style={{ fontFamily: "'Plus Jakarta Sans', sans-serif" }}>
      ...
      <div className="relative">
        {showFilters && (
          <div className="fixed inset-0 z-40 bg-black/5 dark:bg-black/20" onClick={() => setShowFilters(false)} />
        )}
          ...
          <div className="absolute top-0 left-0 w-full z-50 bg-white/80 dark:bg-zinc-950/80 backdrop-blur-xl border border-zinc-100 dark:border-zinc-900 rounded-[2.5rem] p-6 lg:p-8 shadow-2xl animate-in slide-in-from-top-4 duration-500">
            ...
            {hasActiveFilters && (
              <div className="mt-8 pt-4 border-t border-zinc-100 dark:border-zinc-900 flex justify-end">
                <button onClick={resetFilters} className="flex items-center gap-2 text-[10px] font-black text-rose-500 uppercase tracking-widest">
        ...
        <div className="relative pt-2">
          {isLoading ? (
            ...
            // ✅ FIX: Walang onSelect — hayaan ang Cards mag-navigate nang mag-isa
            <Cards
