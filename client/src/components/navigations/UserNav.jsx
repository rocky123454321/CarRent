    ...
    <nav className="bg-white/90 backdrop-blur-md w-full fixed top-0 left-0 z-50 border-b border-slate-200/60 shadow-sm">
      <div className="flex items-center justify-between max-w-7xl mx-auto px-4 sm:px-6 h-16">
        ...
        <div className="hidden md:flex items-center gap-1 absolute left-1/2 -translate-x-1/2">
          <Link to="/" className="px-4 py-2 text-sm font-semibold rounded-xl text-slate-600 hover:bg-slate-50 hover:text-indigo-600 transition-all">
            Home
          </Link>
          <Link to="/cars" className="px-4 py-2 text-sm font-semibold rounded-xl text-slate-600 hover:bg-slate-50 hover:text-indigo-600 transition-all">
            Category
          </Link>
          <Link to="/my-rentals" className="px-4 py-2 text-sm font-semibold rounded-xl text-slate-600 hover:bg-slate-50 hover:text-indigo-600 transition-all">
            My Rentals
          </Link>
          <Link to="/chat" className="px-4 py-2 text-sm font-semibold rounded-xl text-slate-600 hover:bg-slate-50 hover:text-indigo-600 transition-all">
            Chat
          </Link>
        </div>
        ...
        <div className="flex items-center gap-2">
          <div className="hidden md:flex relative w-[180px] lg:w-[240px]">
          ...
          <button
            ...
            className={`md:hidden w-10 h-10 flex items-center justify-center rounded-xl transition-all active:scale-95 ${
              mobileOpen ? 'bg-slate-100 text-slate-600' : 'bg-slate-900 text-white shadow-lg shadow-slate-200'
            }`}
          >
      ...
      {mobileOpen && (
        <>
          {/* Backdrop (Pinapadilim ang likod) */}
          <div 
          ...
          <div className="md:hidden absolute inset-x-0 top-full bg-white border-b border-slate-200 shadow-2xl z-50 animate-in slide-in-from-top duration-300 ease-out">
            <div className="p-5 flex flex-col gap-4">
              ...
              <div className="flex items-center gap-4 p-4 rounded-2xl bg-indigo-50 border border-indigo-100 shadow-sm">
                ...
                <div className="flex-1 min-w-0">
                  ...
                  <p className="text-xs text-indigo-600 font-medium truncate opacity-80">{user?.email}</p>
                </div>
              </div>

              {/* Quick Actions Grid */}
              <div className="grid grid-cols-2 gap-3">
                <button onClick={() => handleMobileNav('/cars')} className="flex flex-col items-center justify-center p-4 rounded-2xl border border-slate-100 bg-white active:bg-slate-50 gap-2 transition-all shadow-sm">
                ...
                <button onClick={() => handleMobileNav('/my-rentals')} className="flex flex-col items-center justify-center p-4 rounded-2xl border border-slate-100 bg-white active:bg-slate-50 gap-2 transition-all shadow-sm">
              ...
              <div className="flex flex-col gap-2">
                <button onClick={() => { setNotifOpen(true); setMobileOpen(false); }} className="flex items-center justify-between p-4 rounded-2xl bg-white border border-slate-100 active:bg-slate-50 transition-all shadow-sm">
                ...
                <button onClick={() => handleMobileNav('/chat')} className="flex items-center justify-between p-4 rounded-2xl bg-white border border-slate-100 active:bg-slate-50 transition-all shadow-sm">
                  <div className="flex items-center gap-3">
                ...
                <button onClick={() => handleMobileNav('/settings')} className="flex items-center justify-between p-4 rounded-2xl bg-white border border-slate-100 active:bg-slate-50 transition-all shadow-sm">
                  <div className="flex items-center gap-3">
                    <div className="w-8 h-8 rounded-lg bg-slate-100 text-slate-600 flex items-center justify-center"><Settings size={18} /></div>
                    <span className="font-semibold text-sm text-slate-700">Settings</span>
                  </div>
                  <ChevronRight size={16} className="text-slate-300" />
                </button>

                <button onClick={logout} className="flex items-center gap-3 p-4 rounded-2xl bg-red-50 text-red-600 font-bold mt-2 shadow-sm border border-red-100 active:scale-[0.98] transition-transform">
        ...
        <AlertDialogContent className="max-w-[calc(100%-2rem)] w-full rounded-3xl p-0 overflow-hidden border-none shadow-2xl">
          ...
          {notifications.length > 0 && (
            <div className="p-4 bg-slate-50">
              <button 
                onClick={() => { handleMobileNav('/chat'); setNotifOpen(false); }} 
                className="w-full py-3.5 bg-indigo-600 text-white rounded-2xl text-xs font-bold shadow-lg shadow-indigo-100 active:scale-[0.98] transition-all"
              >
                GO TO MESSAGES
              </button>
            </div>
