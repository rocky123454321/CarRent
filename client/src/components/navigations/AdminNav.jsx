...
const NotifIcon = ({ type, status }) => {
  if (type === "chat")
    return (
      <div className="w-10 h-10 rounded-full bg-zinc-900 dark:bg-white text-white dark:text-zinc-900 flex items-center justify-center shrink-0 border border-zinc-800 dark:border-zinc-200">
...
const NotifRow = ({ n, onView }) => {
  ...
  const sub =
    n.type === "chat"
      ? n.message
      : n.type === "new-booking"
      ? `${n.carBrand} ${n.carModel} — ₱${n.totalPrice?.toLocaleString()}`
        ...
        <div className="flex-1 min-w-0">
          <div className="flex justify-between items-center mb-0.5">
            ...
            <span className="text-[9px] font-bold text-zinc-400 shrink-0 ml-2">
              {new Date(n.time || n.timestamp).toLocaleTimeString([], {
                hour: "2-digit",
                minute: "2-digit",
              })}
            </span>
          </div>
          <p className="text-[11px] text-zinc-500 dark:text-zinc-400 truncate font-medium leading-snug">
            {sub}
          </p>
        </div>
      </div>
        ...
        <div className="flex items-center gap-3">
          {unreadCount > 0 && (
            <button
              ...
              className="text-[9px] font-bold uppercase tracking-tighter text-zinc-400 hover:text-zinc-700 dark:hover:text-zinc-200 transition-colors"
            >
              Mark all read
            </button>
          )}
          {notifications.length > 0 && (
            <button
              onClick={onClear}
              className="text-[9px] font-bold uppercase tracking-tighter hover:underline text-zinc-400 hover:text-zinc-700 dark:hover:text-zinc-200 transition-colors"
            >
              Clear
            </button>
          )}
      ...
      <div className="max-w-7xl mx-auto px-6 h-[72px] flex items-center justify-between">
        ...
        <div className="flex items-center gap-3">
          ...
          <DropdownMenu>
            ...
            <DropdownMenuContent
              className="w-80 shadow-2xl border-zinc-100 dark:border-zinc-900 p-0 bg-white dark:bg-zinc-950 overflow-hidden rounded-2xl"
              align="end"
              sideOffset={10}
            >
              <NotificationPanel
              ...
              >
                ...
                <DropdownMenuGroup className="p-1.5">
                  <DropdownMenuItem
                    ...
                    className="rounded-lg py-2.5 px-4 cursor-pointer flex items-center gap-3 text-zinc-500 hover:text-zinc-900 dark:focus:text-white focus:bg-zinc-50 dark:focus:bg-zinc-900 font-bold text-[10px] uppercase tracking-widest"
                  >
              ...
              {["Dashboard", "Chat", "Bookings", "Settings"].map((item) => (
                ...
                >
                  ...
                  {item === "Chat" && totalUnread > 0 && (
                    <span className="text-sm bg-zinc-900 dark:bg-white text-white dark:text-zinc-900 rounded-full px-2 py-0.5 font-black">
                      {totalUnread}
                    </span>
                  )}
        ...
        <AlertDialogContent className="max-w-[calc(100%-2rem)] w-full rounded-[2.5rem] p-0 overflow-hidden border border-zinc-100 dark:border-zinc-900 shadow-2xl bg-white dark:bg-zinc-950">
          <AlertDialogHeader className="px-6 py-5 border-b border-zinc-50 dark:border-zinc-900">
            <div className="flex items-center justify-between w-full">
              <AlertDialogTitle className="text-[10px] font-black uppercase tracking-[0.3em] text-zinc-400">
                Activity
              </AlertDialogTitle>
          ...
          <div className="p-5 border-t border-zinc-50 dark:border-zinc-900">
            <button
              ...
              className="w-full py-4 bg-zinc-950 dark:bg-white text-white dark:text-zinc-950 rounded-2xl text-[10px] font-black uppercase tracking-[0.2em] active:scale-[0.98] transition-all"
            >
