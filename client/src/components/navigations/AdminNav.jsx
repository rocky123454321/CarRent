...
const AdminNav = ({ onMenuClick }) => {
  ...
  return (
    <header className="sticky top-0 z-30 bg-white/80 backdrop-blur-md border-b border-slate-100 px-6 py-3.5 flex items-center gap-4 shadow-sm">

      {/* Sidebar toggle */}
      <button
        onClick={onMenuClick}
        className="lg:hidden w-9 h-9 flex items-center justify-center rounded-xl border border-slate-200 text-slate-500 hover:text-slate-800 transition"
      >
        <Menu size={18} />
      </button>
      ...
      <div className="flex items-center gap-2">
          ...
          <DropdownMenuContent className="w-80 rounded-2xl shadow-xl border p-0 overflow-hidden" align="end">
            ...
            {notifications.length > 0 && (
              <div className="px-4 py-2.5 border-t bg-gray-50">
                <button onClick={() => navigate('/admin/chat')} className="w-full text-xs text-center text-blue-600 hover:text-blue-800 font-medium">
                  Open Chat
                </button>
              </div>
            )}
            ...
            <DropdownMenuLabel className="px-4 py-4 bg-slate-50 border-b border-slate-100">
              <div className="flex items-center gap-3">
                ...
                <div className="min-w-0">
                  ...
                  <span className="inline-block mt-0.5 px-2 py-0.5 rounded-full bg-indigo-50 text-indigo-600 text-[10px] font-semibold">Admin</span>
                </div>
              </div>
            </DropdownMenuLabel>
        ...
        <button
          ...
          className="sm:hidden w-9 h-9 flex items-center justify-center rounded-xl border border-slate-200 text-slate-500 transition"
        >
      ...
      {/* Mobile Menu */}
      {mobileOpen && (
        <div className="sm:hidden absolute top-full left-0 right-0 bg-white/95 backdrop-blur-xl border-b border-slate-200 shadow-xl px-5 py-5 flex flex-col gap-4">

         
          {/* Mobile Alerts → AlertDialog */}
      ...
      <AlertDialog open={notifOpen} onOpenChange={setNotifOpen}>
        <AlertDialogContent className="rounded-2xl w-[90vw] max-w-sm p-0 overflow-hidden">
          ...
          <AlertDialogFooter className="px-4 py-3 border-t bg-gray-50 flex flex-row gap-2">
            {notifications.length > 0 && (
              <button
                onClick={() => { navigate('/admin/chat'); setNotifOpen(false); }}
